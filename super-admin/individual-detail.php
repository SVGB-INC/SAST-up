...
$userLogoQuery = "SELECT user_img from dt_user_info where user_ID = '" . $_SESSION["user_id"] . "';";
$userLogoList = $pdo->query($userLogoQuery)->fetch();
$subID = $_GET["uid"];

//getting subscriber details
$subQuery = "SELECT * from dt_user_info where uID = '" . $subID . "' LIMIT 1;";
                                ...
                                <div class="d-flex justify-content-between align-items-lg-baseline">
                                    <div>
                                        ...
                                        <label class="fs-4 fw-bolder"><?php
                                                                        ...
                                                                        if ($bookNow) { ?>
                                                <a href=<?= "../category-note.php?bid=" . $bookNow["uID"] ?>><?= $bookNow["book"] ?></a>
                                            <?php } else { ?>
                                                <?php
                                                                            //get parent UID
                                                                            $parentUID = $pdo->query("SELECT company_ID from dt_subscribers WHERE uID = '" . $_SESSION["user_id"] . "';")->fetchColumn();
