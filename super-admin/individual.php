...
$individualsQuery = "SELECT *, a.uID as uID, a.user_email as email from dt_users_individual as a left join dt_user_info as b on b.user_ID=a.uID";
$IndividualsList = $pdo->query($individualsQuery)->fetchAll();
...
if ($_SERVER['REQUEST_METHOD'] === "POST") {
    ...
    if ($formName == "newform") { //this request is for inserting categories
        ...
        try{
            ...
            $select_stmt = $pdo->prepare("SELECT user_name FROM dt_users_individual WHERE user_name=:u_name");
            ...
            if ($select_stmt->rowCount() > 0) {
                ...
                $select_stmt = $pdo->prepare("SELECT user_email FROM dt_users_individual WHERE user_name=:u_email");
                $select_stmt->execute(array(
                ':u_email' => $user_email,
                ));
                $row = $select_stmt->fetch(PDO::FETCH_ASSOC);
                ...
                if ($select_stmt->rowCount() > 0) {
                    ...
                    $insertUser = $pdo->prepare("INSERT INTO dt_users_individual(`user_name`, `user_pass`, `user_email`, `user_full_name`) VALUES (?,?,?,?)");
                
                    if ($insertUser->execute([
                        $user_name, password_hash($user_pass, PASSWORD_DEFAULT), $user_email, $user_full_name
                    ])) {
                        $id = $pdo->lastInsertId();
                        //insert the new unique user into the DB in user_ino as well.
                        $insert_stmt = $pdo->prepare("INSERT INTO dt_user_info(user_ID,first_name,last_name,user_email,user_employer) VALUES (?,?,?,?,?)");

                        $insert_stmt->execute([$id,$first_name,$last_name,$user_email,$user_full_name]);
...
?>
            ...
            <div class="page-wrapper subscriber mt-3">
                <div class="container-fluid">
                    <div class="d-flex align-items-center mb-4">
                        <h5 class="fw-normal me-5 text-primary">Individuals</h5>
                        <p nowrap></p>
                    </div>
                    <?php if ($error_msg != "NA") { ?>
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-12 col-md-4 col-lg-3 col-xl-2 my-1">
                            <button class="btn btn-primary w-100" data-bs-toggle="modal" data-bs-target="#exampleModal">New Individual</button>
                        </div>
                    </div>
                    <div class="table-responsive my-5" style="height: 50vh;">
                        ...
                        <table class="table align-middle" id="example">
                            ...
                            <tbody>
                                <?php 
                                //echo "<pre>"; print_r($IndividualsList); exit;
                                    ...
                                    <tr>
                                        <th scope="row" class="d-flex align-items-center">
                                            ...
                                            <span class="d-flex align-items-center">
                                                <img src=<?php if (empty($Individual["user_img"]) || $Individual["user_img"] == "NA") {
                                                                echo "./assets/images/user_placeholder.png";
                                                            } else {
                                                                echo "../" . $Individual["user_img"];
                                                            } ?> alt="user" class="img-fluid" width="50" />
                                            </span>
                                        </th>
                                        <td><a href='individual-detail.php?uid=<?= $Individual["uID"] ?>' class="text-secondary"><?= $Individual["first_name"] ?> <?= $Individual["last_name"] ?></a></td>
                                        <td><a href='individual-detail.php?uid=<?= $Individual["uID"] ?>' class="text-secondary"><?= $Individual["user_name"] ?? '' ?></a></td>
                                        <td><a href='individual-detail.php?uid=<?= $Individual["uID"] ?>' class="text-secondary"><?= $Individual["email"] ?? '' ?></a></td>
                                        <td><?= $Individual["user_employer"] ?? '' ?></td>
                                        <td>
                                            <div class="edit-delete">
                                                <!-- <span class="pe-3 delete"><i class="bi bi-trash3" onclick="deleteIndividual(<?= $Individual['uID'] ?>)"></i></span> -->
                                                <button class="btn btn-primary" onclick="deleteIndividual(<?= $Individual['uID'] ?>,<?= $Individual['user_ID'] ?>)"><i class="bi bi-trash3"></i></button>
                                            </div>
                                        </td>
                                    </tr>
                                <?php } ?>
                            ...
                            <div class="modal-content p-5" style="border-radius:20px;">
                                <form method="POST" enctype="multipart/form-data">
                                    <div class="model-head">
                                        <h4 class="fw-normal">New Individual User</h4>
                                    </div>
        ...
        <script>
            ...
            function deleteIndividual(id,uid) {
                var result = confirm("Are you sure you want to delete this Individual?");
                if (result) {
                    $.ajax({
                        type: "GET",
                        ...
                        success: function(data) {
                            if (data == "deleted") {
                                alert("This Individual was successfully deleted!");
